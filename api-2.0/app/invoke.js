...
const invokeTransaction = async (channelName, channelDestination, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        const ccp = await helper.getCCP(org_name);
        console.log("==================", channelName, channelDestination ,chaincodeName, fcn, args, username, org_name,)
        ...
        switch (fcn) {
            case "createData":
                ...
                break;
            case "saveSummedData":
                result = await contract.submitTransaction(fcn, channelDestination, args[0] );
                result = {txid: result.toString()}
                break;
            case "saveSummedDataNations":
                result = await contract.submitTransaction(fcn, args[0] );
                result = {txid: result.toString()}
                break;
            default:
